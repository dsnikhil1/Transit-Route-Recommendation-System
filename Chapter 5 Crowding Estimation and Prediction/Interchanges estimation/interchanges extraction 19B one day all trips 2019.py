                        ...
                        for i in range(np.shape(all_stages_route)[0]):
                            ...
                            for j in range(np.shape(all_stages_route)[0]):
                                if i == j:
                                    ...
                                    df_tickets_data_source_destination_schedule_trip_stage_i = df_tickets_data_source_destination_schedule_trip[df_tickets_data_source_destination_schedule_trip['FromStage']==all_stages_route[i]]
                                    df_tickets_data_source_destination_schedule_trip_stage_i_stage_j = df_tickets_data_source_destination_schedule_trip_stage_i[df_tickets_data_source_destination_schedule_trip_stage_i['ToStage']==all_stages_route[j]]
                                    adult_sum = df_tickets_data_source_destination_schedule_trip_stage_i_stage_j['Adult'].sum()
                                    child_sum = df_tickets_data_source_destination_schedule_trip_stage_i_stage_j['Child'].sum()
...
DF.to_csv("C:/Users/DSNikhil/Downloads/Codes and Corresponding files/Chapter 5 Crowding Estimation and Prediction/Interchanges estimation/Stage to Stage interchnages for all 19B trips on 20-05-2019.csv",index=True)
